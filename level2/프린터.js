...
const solution = (priorities, location) => {
    ...
    const printArray = new Array(priorities.length).fill(0).map((item, idx) => idx);
    const target = printArray[location];
    
    while(printArray.length) {
        const minValue = Math.max(...priorities);
   
        if(minValue === priorities[0]) {
            sortArray.push(printArray.shift());
            priorities.shift();
        } else {
            priorities.push(priorities.shift());
            printArray.push(printArray.shift());
        }
    };
    
    return sortArray.indexOf(target) + 1;
};
